...
static u32 g_dword_xor_first;           /* 1回目のReBAR CTRL書込みに掛けるxor(resize) */
static u32 g_dword_xor_rest;            /* 2回目以降のReBAR CTRL書込みに掛けるxor(復元) */
...
static int pci_write_config_dword(struct pci_dev *d, int off, u32 v)
{
	if (is_rebar_ctrl_off(d, off)) {
		...
		if (g_rebar_writes == 1)
			v ^= g_dword_xor_first; /* resize書込みの化け(T17) */
		else
			v ^= g_dword_xor_rest;  /* 復元書込みの化け(T17) */
	}
...
static void reset_counters(void)
{
	...
	g_dword_xor_first = 0; g_dword_xor_rest = 0;
...
int main(void)
{
	...
	printf("T17: 復元がsize温存でsize外ビットだけ化けても「restored」と誤判定しない\n");
	reset_counters();
	d = mkdev(0x7fc0, 1, 8, 1, 0x0006);
	/* resize書込みはsizeビットが化ける(6→4着弾=不一致を作る)。
	   復元書込みはbar_idxビット(size外)だけ化ける: 着弾値はsize=8で旧sizeと同じ。
	   → 旧実装(sizeフィールドだけ比較)なら「restored」と誤判定して素通りする。
	   → 現実装(CTRL全体一致)は必ず「restore NOT verified」を出す。
	   この試験は全体一致検証がsize比較へ退行したら必ずFAILする(Codex P2対応)。 */
	g_dword_xor_first = 2u << PCI_REBAR_CTRL_BAR_SHIFT;	/* size 6→4 */
	g_dword_xor_rest = 0x00000002u;				/* bar_idx 1→3 (size不変) */
	quirk_shot0_nvidia_bar1_64mib(&d);
	chk(cur_size(&d, 0) == 8, "着弾した復元値のsizeは旧値と同じ(=旧実装なら素通りの形)");
